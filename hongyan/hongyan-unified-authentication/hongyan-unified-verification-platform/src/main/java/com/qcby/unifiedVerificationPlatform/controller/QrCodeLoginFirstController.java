...
import com.qcby.unifiedVerificationPlatform.entity.CodeLoginKey;
import com.qcby.unifiedVerificationPlatform.properties.WxConfig;
import com.qcby.unifiedVerificationPlatform.util.CodeLoginUtil;
import com.qcby.unifiedVerificationPlatform.util.HttpClientUtil;
...
public class QrCodeLoginFirstController {
    ...
    private ResultJson getQrCode() {
        try {
            ...
            String result = HttpClientUtil.doPostJson(getQrCodeUrl, json);//从这一行进行了一个网络post请求(连接了网络)
            JSONObject jsonObject = JSONObject.parseObject(result);//从这一行开始进行了网页前端输出
    ...
    public String checkSign(HttpServletRequest request) throws Exception {
        ...
        String[] params = new String[]{timestamp, nonce, "4647d5451d1f42e8"};
        Arrays.sort(params);
        //拼接
    ...
    public JSONObject callback(HttpServletRequest request) throws Exception {
        ...
        jsonObject.put("code", "200");//微信应该发过来一个code,我们验证是否为200,如果为200,则通过,进行下一步
